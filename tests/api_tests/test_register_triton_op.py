...
SECTION 4: Dataclass & End-to-End Tracing
  - Dataclass inputs navigating through to Triton kernels
  - Nested dataclass & backward combination
...
# Module-level frozen dataclass used by the dataclass+triton test below. We
# declare it at module scope (not inside the test method) so that
# ``typing.get_type_hints`` / ``eval`` on the function's stringified
# annotations (PEP 563 / ``from __future__ import annotations``) can find it
# via ``fn.__globals__``.
...
# Nested dataclass fixtures for the nested-dataclass + triton tests below.
# Same module-scope rationale as ``_DcCosCfg``.
...
class TestMultiLevelNesting:
    ...
    def test_introspection_walks_all_levels(self):
        ...
        kernels = get_inner_triton_kernels(fn)
...
# Nested dataclass + triton kernel: dataclass-of-dataclass arguments must be
# fully flattened so that ``infer_schema`` only sees primitive types, while
# the triton_op path still sees through the kernel.
...
class TestExtraTritonKernelsDedup:
    def test_dedup_in_resolve_and_rewrite(self):
        from magi_compiler._magi_register_custom_op import _resolve_triton_kernels
        from magi_compiler._triton_introspect import rewrite_fn_with_wrap_triton
...
# Dataclass input + triton kernel inside the body. Verifies the dataclass-
# aware path also routes ``inner_fn`` through ``rewrite_fn_with_wrap_triton``
# when triton kernels are present.
...
# #21: dataclass input + is_compute_sensitive on triton path
...
class TestIntrospection:
    def test_get_inner_triton_kernels_flat(self):
        from magi_compiler._triton_introspect import get_inner_triton_kernels
    ...
    def test_get_inner_triton_kernels_nested(self):
        from magi_compiler._triton_introspect import get_inner_triton_kernels
...
class TestDataclassWithTritonKernel:
    def test_dataclass_input_with_triton(self):
        # Use the module-level _DcCosCfg dataclass declared near the top of
        # this test module so the type is visible from ``fn.__globals__``
        # even with ``from __future__ import annotations`` in effect (which
        # turns every annotation into a string at runtime).
...
# nn.Module owning the kernel as ``self.kernel``: the introspector cannot
# see through ``self.kernel[grid](...)`` (Subscripted Attribute, not a plain
# Name), so the user must use ``extra_triton_kernels``. We assert that the
# escape hatch makes this work end-to-end.


class TestNestedDataclassWithTritonKernel:
    def test_two_level_nested_dc_with_triton(self):
        """Outer dataclass containing an inner dataclass; both are unwrapped
        into ``cfg__kernel__block_size``, ``cfg__kernel__extra_offset`` and
        ``cfg__scale`` flat parameters."""
        ...
        # Sanity-check the flat plan.
        plan = fn._magi_flat_plan
        cfg_node = plan[1]
...
# Triton kernel + dataclass input + backward_fn: the dataclass-aware path
# must bridge ``setup_context`` and ``backward`` so that the user can keep
# writing them against the ORIGINAL signature (with the dataclass instance)
# while underneath the registered op is flat.
...
class TestDataclassWithTritonKernelAndBackward:
    def test_triton_dc_backward_basic(self):
        ...
        (analytical grad: -sin(x) * cfg.factor) so we can verify exact grads."""
    ...
    def test_triton_dc_backward_with_dict_grad(self):
        """User returns the dataclass slot's grad as a plain ``dict`` (handy
        when constructing another instance is awkward, e.g. in a generic
        backward). The bridge must spread it through ``__getitem__``-style
        access into the underlying flat slots."""
        ...
        def _bwd(ctx, grad_out):
            (x,) = ctx.saved_tensors
            # Use a dict for the dc slot rather than constructing a new
...
# Module-level @triton.heuristics fixtures for the rejection tests below.
# Defined at module scope so triton.jit's source-resolution machinery (which
# needs a real .py file) succeeds.
