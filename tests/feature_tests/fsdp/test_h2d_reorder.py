...
from magi_compiler.passes.fsdp_overlap import h2d_reorder
from magi_compiler.passes.fsdp_overlap.h2d_reorder import H2dLoadReorder
...
def test_two_loads_do_not_spend_the_same_compute():
    ...
    # ...and their shards are never live together.
...
def test_only_one_shard_is_ever_live():
    ...
    ld0's shard stays allocated until w0 reads it.  Hoisting ld1 above w0 would
    put two shards on the device at once, and the hoists compound -- which is how
    an unconstrained sweep rebuilds, on the device, the residency that offloading
    just paid PCIe to remove.  ld1 is handed back instead.
    """
    ...
    try:
        ...
        # ld0's shard would then be live at the same time, so ld0 goes resident.
...
def test_residency_cap_pulls_a_weight_back_into_an_idle_window():
    ...
    The sweep keeps ld0 resident because its shard would overlap ld1's.  Under a
    cap that forbids keeping it, it must come back into the offload plan -- into
    a window the sweep already left idle, where the bus is free.
    """
    from magi_compiler.offload import host_pool
...
def test_loads_far_enough_apart_both_survive():
    ...
    With ld0's gather well upstream of where ld1 lands, the two shards are never
    live together and both stay offloaded -- the rule must not cost residency it
    does not have to.
    """
    from magi_compiler.offload import host_pool
...
def test_live_ranges_never_overlap_however_tight_the_chain():
    ...
    Whatever the pass decides to hoist, no two surviving shards may be live at
    the same time -- a shard is live from where its load runs until its wait
    reads it.  Checking the invariant beats checking a promotion count: the count
    depends on where the sweep happens to land, the invariant is the contract.
    """
    from magi_compiler.offload import host_pool
