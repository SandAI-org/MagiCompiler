...
// Single-kernel fully-fused swiglu7 on Hopper (sm_90a) using the vendored
...
// sibling at ../../sm80/cutlass_kernels/swiglu7_one_stage.cu — same Python
// signature, same B gate/linear interleaved layout (ldB = 2K col-major
// view), same Sw7Args shape, same stride-based input checks.
//
//   D = swiglu7(A @ B.T)
//
...
// evt_runtime.py::_compile_swiglu7_dual when the live device's compute
...
// Greedy-picked on the host side via -DMAGI_SWIGLU7_ALIGN_*_BITS — same macro
...
// Same Sw7Args layout as the sm_80 path — keeps the host wrapper identical.
////////////////////////////////////////////////////////////////////////////////
...
class Sw7Sm90Concept {
 public:
  virtual ~Sw7Sm90Concept() = default;
...
template <class Cfg>
class Sw7Sm90Impl : public Sw7Sm90Concept {
 public:
  ...
  explicit Sw7Sm90Impl(const char* name) : name_(name) {}
...
class Sw7Sm90AutoTuneRunner {
 public:
  Sw7Sm90AutoTuneRunner() {
    ...
    SW7_SM90_TILE(128, 128, 64, 4, "Sm90<128,128,64>_S4");   // 192 KiB
    SW7_SM90_TILE(128, 128, 64, 3, "Sm90<128,128,64>_S3");   // 144 KiB

    // ── Decode-style small M ─────────────────────────────────────────────────
    SW7_SM90_TILE(64,  128, 64, 4, "Sm90<64,128,64>_S4");    // 160 KiB
    SW7_SM90_TILE(64,   64, 64, 4, "Sm90<64,64,64>_S4");     //  96 KiB

    // ── Alternate small-N ────────────────────────────────────────────────────
    SW7_SM90_TILE(128,  64, 64, 4, "Sm90<128,64,64>_S4");    // 128 KiB
  ...
  void operator()(at::Tensor A, at::Tensor B, at::Tensor D,
    ...
    // ). Express in bytes so a future fp8 / fp32 swiglu7 path inherits the
    ...
    TORCH_CHECK((K % kMinKAlignElems) == 0,
                "Sm90 swiglu7 requires K * sizeof(elem) % 16 == 0 (TMA's 128-bit "
 ...
 private:
  ...
  std::vector<std::unique_ptr<Sw7Sm90Concept>> configs_;
  int best_idx_ = -1;     // -1 = not yet autotuned; sticky after first call.
  at::Tensor ws_;
};

static Sw7Sm90AutoTuneRunner& runner() {
  static Sw7Sm90AutoTuneRunner R;
  return R;
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.doc() = "CUTLASS Sm90 DualGemm fully-fused swiglu7 (bf16) on sm_90a — autotune";
  m.def("swiglu7_dual_matmul_out",
        &swiglu7_dual_matmul_out,
        "D = swiglu7(A @ B.T) in a single fused Sm90 (TMA+WGMMA) kernel; "
